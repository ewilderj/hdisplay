...
// Flags and reserved names are provided via CJS; import as default and destructure
import flagsPkg from './flags.js';
const { EXCLUDED_DATA_FLAGS } = flagsPkg;
...
async function promptSelectService(services) {
  ...
  while (true) {
    const answer = (
      await ask(chalk.cyan('Select a server [1-' + services.length + '] (default 1): '))
    ).trim();
    if (answer === '') {
      choice = 1;
      break;
...
async function api(pathname, method = 'get', data) {
  ...
  try {
    ...
    if (e.response) {
      ...
      throw new Error(
        `HTTP ${e.response.status} ${e.response.statusText}${msg ? ` - ${msg}` : ''}`
      );
    }
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (valueProvided) {
      ...
      if (i + 1 < tokens.length && !tokens[i + 1].startsWith('--')) {
        i++;
        setDeep(data, path, coerceValue(tokens[i]));
      } else {
...
async function resolveDiscover() {
  const override = process.env.HDISPLAY_DISCOVER_IMPL;
  if (override) {
    const spec = pathToFileURL(override).href;
    const mod = await import(spec);
    return mod.default || mod;
  }
  const mod = await import('./commands/discover.mjs');
  return mod.default;
}

export default async function run(argv = []) {
  const discover = await resolveDiscover();
  ...
  program
    ...
    .action((opts) => {
      ...
      if (opts.server) {
        ...
        saveConfig(cfg);
        console.log(chalk.green('Updated server to'), cfg.server);
      }
      console.log(cfg);
    });
  ...
  program
    ...
    .action(async (htmlParts) => {
      ...
      if (!html) {
        console.error(chalk.red('Error: HTML content required.'));
        process.exitCode = 1;
        return;
      }
      try {
        await api('/api/content', 'post', { content: html });
        successLog(chalk.green('Content updated'));
      } catch (e) {
        console.error(chalk.red('Error:'), e.message);
        process.exitCode = 1;
      }
  ...
  program
    ...
    .action(async (msgParts, opts) => {
      ...
      if (!Number.isFinite(duration) || duration < 0) {
        console.error(chalk.red('Error: duration must be a non-negative integer.'));
        process.exitCode = 1;
        return;
      }
      if (!ALLOWED_LEVELS.has(level)) {
        console.error(chalk.red('Error: level must be one of info|warn|error|success.'));
        process.exitCode = 1;
        return;
      }
      try {
        await api('/api/notification', 'post', { message, duration, level });
        successLog(chalk.green('Notification sent'));
      } catch (e) {
        console.error(chalk.red('Error:'), e.message);
        process.exitCode = 1;
      }
    ...
    .action(async () => {
      try {
        ...
        if (!data.templates || data.templates.length === 0) {
          console.log(chalk.yellow('No templates found. Add .html files in templates/'));
          return;
        }
        data.templates.forEach((t) =>
          console.log(
            chalk.cyan(t.id),
            '-',
            t.placeholders && t.placeholders.length ? t.placeholders.join(', ') : '(no vars)'
          )
        );
      } catch (e) {
        console.error(chalk.red('Error:'), e.message);
  ...
  program
    ...
    .action(async () => {
      try {
        ...
        successLog(chalk.green('Display cleared'));
      } catch (e) {
        console.error(chalk.red('Error:'), e.message);
        process.exitCode = 1;
      }
  ...
  program
    ...
    .action(async (opts) => {
      try {
        ...
        if (!list.length) {
          console.log(chalk.yellow('No hdisplay servers found on the network.'));
          return;
        }
        if (opts.set) {
          let selected;
          if (list.length > 1) {
            selected =
              opts.nonInteractive || !process.stdin.isTTY ? list[0] : await promptSelectService(list);
          } else {
            selected = list[0];
          }
          const cfg = loadConfig();
  ...
  program
    ...
    .action(async (opts) => {
      if (!opts.file && !opts.url) {
        console.error(chalk.red('Error: --file or --url required'));
        process.exitCode = 1;
        return;
      }
      try {
        const res = await pushMedia('/api/push/image', opts);
        successLog(chalk.green('Image displayed at'), res.url);
      } catch (e) {
        console.error(chalk.red('Error:'), e.message);
        process.exitCode = 1;
      }
  ...
  program
    ...
    .action(async (opts) => {
      ...
      try {
        const res = await pushMedia('/api/push/video', opts);
        successLog(chalk.green('Video displayed at'), res.url);
      } catch (e) {
        console.error(chalk.red('Error:'), e.message);
        process.exitCode = 1;
      }
    ...
    .action(async () => {
      try {
        ...
        data.items.forEach((it, i) =>
          console.log(`[${i}]`, it.id, it.data ? JSON.stringify(it.data) : '')
        );
      } catch (e) {
        console.error(chalk.red('Error:'), e.message);
        process.exitCode = 1;
  ...
  program
    .command('playlist:remove <indexOrId>')
  ...
  program
    .command('playlist:delay <ms>')
    .description('Set dwell per item (2000–300000 ms)')
    .action(async (ms) => {
      const n = Number(ms);
      if (!Number.isFinite(n) || n <= 0) {
        console.error(chalk.red('Error: ms must be a positive integer'));
        process.exitCode = 1;
        return;
      }
      try {
  ...
  program
    ...
    .action(async (opts) => {
      try {
        const { default: BlackBoxCapture } = await import('../capture/capture.js');
  ...
  program
    .command('capture:template <templateId>')
  ...
  // Parse provided argv; program expects a full argv with node+script, but accepts sub-array in practice
  await program.parseAsync(['node', 'hdisplay', ...argv]);
}
