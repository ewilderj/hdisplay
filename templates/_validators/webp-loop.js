...
module.exports = function validate(data = {}) {
  ...
  if (data.fit && !['cover','contain'].includes(String(data.fit))) return 'fit must be "cover" or "contain"';
  if (data.rendering && !['pixelated','auto',''].includes(String(data.rendering).toLowerCase())) {
    return 'rendering must be "pixelated" or "auto"';
  }
  if (data.pixelated != null && typeof data.pixelated !== 'boolean') return 'pixelated must be boolean';
  return true;
};
